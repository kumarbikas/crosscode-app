...
class NetworkDiagram1 extends React.Component {
	...
	/*var data = { "machineid": "1234", "machinename": "Crosscode1", "ipAddress": "10.133.144.167", "processList": [ { "processid": "1", "processname": "process-1" }, { "processid": "2", "processname": "process-2" }, { "processid": "3", "processname": "process-3" },{ "processid": "4", "processname": "process-4" },{ "processid": "5", "processname": "process-5" } ] }
	*/
	var data = { "machineid": "1234", "machinename": "Crosscode1", "ipAddress": "10.133.144.167", "processList": [ { "processid": "process-1", "processname": "process-1", "established":[ "process-2","process-3" ], "listen":[ "process-4","process-5" ] }, { "processid": "process-2", "processname": "process-2", "established":[ "process-1","process-3" ], "listen":[ "process-4","process-5" ] }, { "processid": "process-3", "processname": "process-3", "established":[ "process-2","process-1" ], "listen":[ "process-4","process-5" ] }, { "processid": "process-4", "processname": "process-4", "established":[ "process-2","process-3" ], "listen":[ "process-1","process-5" ] }, { "processid": "process-5", "processname": "process-5", "established":[ "process-3","process-4" ], "listen":[ "process-1","process-2" ] } ] }
		...
		var y = 20;
		   ...
		   posIndex = posIndex + 1;
			...
			link(parent, child, [{x: 250, y: 200}]);
					...
					/*$.each(process,function(key,connection){
						console.log(key);
						//console.log("connection::::: "+key.length);
						if(key == "established" && connection.length>0){
							$.each(connection,function(index,estItem){
								console.log("established ::: "+process.processname + ","+estItem);
								link(child, estItem, [{x: 250, y: 200}]);
								
							});
						}
						if(key == "listen" && connection.length>0){
							$.each(connection,function(index,lstItem){
								console.log("listen ::: "+process.processname + ","+ lstItem);
								link(child, lstItem, [{x: 250, y: 200}]);
							});
						}
					});
					*/
	...
	$.each(data.processList,function(key,connection){
		...
		if(connection.established.length>0){
			$.each(connection.established,function(index,estItem){
				console.log("established ::: "+connection.processid + ","+estItem);
		...
		if(connection.listen.length>0){
			$.each(connection.listen,function(index,lstItem){
				console.log("listen ::: "+connection.processid + ","+ lstItem);
				console.log(child_arr[lstItem]);
